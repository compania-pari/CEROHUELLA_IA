...
Usar GitHub Actions como unico motor CI/CD para Cero Huella IA, desplegando en Azure Container Apps con promocion `dev -> qa -> prod`.
...
| `main` | Rama estable | Promocion a `qa` y luego `prod` con aprobacion. |
...
Se ejecuta en pull request y push hacia `develop` o `main`.
...
push main
 ...
 -> promocion de la misma imagen a prod
 -> smoke test /health
```
...
El repositorio destino es `compania-pari/CEROHUELLA_IA`. La rama de trabajo debe publicarse y revisarse mediante pull request antes de aplicar infraestructura real.
