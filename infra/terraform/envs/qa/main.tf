...
resource "azurerm_virtual_network_peering" "qa_to_shared_container_apps" {
  count = var.create_container_apps_environment ? 0 : 1
...
resource "azurerm_virtual_network_peering" "shared_container_apps_to_qa" {
  count = var.create_container_apps_environment ? 0 : 1
...
resource "azurerm_private_dns_zone_virtual_network_link" "postgresql_shared_container_apps" {
  ...
  depends_on = [
    module.postgresql,
    azurerm_virtual_network_peering.qa_to_shared_container_apps,
    azurerm_virtual_network_peering.shared_container_apps_to_qa
  ]
}
